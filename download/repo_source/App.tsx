...
const KEYBOARD_SHORTCUTS = [
  ...
  { category: 'View', keys: 'Ctrl+Shift+M', label: 'Problems' },
];
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '10px 20px', borderBottom: '1px solid var(--vscode-border)', flexShrink: 0, backgroundColor: 'var(--vscode-sidebar-bg)', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <input
            ...
            className="w-full max-w-sm bg-[var(--vscode-input-bg)] border border-[var(--vscode-input-border)] text-[var(--vscode-fg)] text-[12px] px-3 py-1.5 rounded outline-none placeholder:opacity-40"
          />
        </div>
        <button onClick={onClose} className="px-3 py-1 text-[12px] bg-[var(--vscode-button-bg)] text-white rounded hover:opacity-90">Close</button>
      </div>
              ...
              <tbody>
                {KEYBOARD_SHORTCUTS.filter((s) => s.category === cat).map((s) => (
                  <tr key={s.keys} style={{ borderBottom: '1px solid var(--vscode-border)' }}>
                    ...
                    <td style={{ padding: '6px 0', fontSize: 12, color: 'var(--vscode-fg)', opacity: 0.7, textAlign: 'right', fontFamily: 'monospace' }}>
                      {s.keys}
                    </td>
                  </tr>
                ))}
...
export default function App() {
  ...
  // Zen mode keyboard shortcut (Ctrl+K Z handled here since it's a chord)
  useEffect(() => {
    let lastK = false;
  ...
  // Listen for vscode:command events from menus / palette
  useEffect(() => {
    const handler = (e: Event) => {
      const { command, ...rest } = (e as CustomEvent).detail;
      ...
      if (command === 'goToLine' && rest.line) {
        // Dispatch to Monaco via custom event
        const monacoEvent = new CustomEvent('monaco:goToLine', { detail: { line: rest.line } });
        window.dispatchEvent(monacoEvent);
      }
    };
