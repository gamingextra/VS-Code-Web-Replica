      ...
      {segments.map((seg, i) => (
        <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 2, minWidth: 0, flexShrink: i < segments.length - 1 ? 1 : 0 }}>
          ...
          <span
            style={{
              fontSize: 12,
              color: i === segments.length - 1 ? 'var(--vscode-fg)' : 'var(--vscode-fg)',
...
function SplitPane({ split, splitIndex, isActive, showDivider, showClose }: SplitPaneProps) {
  ...
  // Listen for format document command
  useEffect(() => {
  ...
  const handleMount = useCallback((editor: monaco.editor.IStandaloneCodeEditor, monacoInstance: typeof monaco) => {
    ...
    // Register themes once per Monaco instance
  ...
  return (
    ...
    >
      {/* Per-split mini tab bar (only shown in multi-split mode) */}
...
export function EditorArea() {
  ...
  // Check if any tabs exist at all
