      ...
      <div style={{ display: 'flex', gap: 64, flexWrap: 'wrap', justifyContent: 'center' }}>
        <div style={{ minWidth: 200 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <WelcomeLink icon={<NewFileIcon size={16} />} label="New File..." shortcut="Ctrl+N" onClick={newUntitled} />
            <WelcomeLink icon={<OpenFolderIcon size={16} />} label="Open Folder..." shortcut="Ctrl+K Ctrl+O" onClick={() => {}} />
            <WelcomeLink icon={<GitCloneIcon size={16} />} label="Clone Repository..." onClick={() => {}} />
          </div>
        </div>
...
function WelcomeLink({
  ...
  return (
    ...
    >
      ...
      {shortcut && <span style={{ fontSize: 11, opacity: 0.4, marginLeft: 8 }}>{shortcut}</span>}
    </button>
  );
}
