...
export const workspaceStore = {
  ...
  listDir(absPath: string): string[] {
    const rel = stripBase(absPath);
    const state = useFileSystemStore.getState();
    if (!rel) return state.root.map(n => n.name);
    const node = state.findNodeByPath(rel);
    if (node?.type === 'folder' && node.children) return node.children.map(c => c.name);
    return [];
  },
  ...
  setContent(absPath: string, content: string): void {
    ...
    if (existing) {
      ...
      if (!parentRel) {
        ...
        parentId = parentNode ? parentNode.id : 'folder-src';
      }
