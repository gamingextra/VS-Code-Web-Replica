...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  openFile: (path, name, content, language) => {
    ...
    if (existing) {
      ...
      if (!splits[splitIdx].tabIds.includes(existing.id)) {
        splits[splitIdx] = { ...splits[splitIdx], tabIds: [...splits[splitIdx].tabIds, existing.id] };
      }
      splits[splitIdx] = { ...splits[splitIdx], activeTabId: existing.id };
      set({ splits });
      return;
    }
    ...
    splits[splitIdx] = {
      ...splits[splitIdx],
      tabIds: [...splits[splitIdx].tabIds, newTab.id],
  ...
  setActiveTab: (tabId) => {
    const splits = [...get().splits];
    const splitIdx = get().activeSplitIndex;
    splits[splitIdx] = { ...splits[splitIdx], activeTabId: tabId };
    set({
      activeTabId: tabId,
  ...
  newUntitled: () => {
    ...
    splits[splitIdx] = {
      ...splits[splitIdx],
      tabIds: [...splits[splitIdx].tabIds, id],
      activeTabId: id,
    };
    set({
      tabs: [...get().tabs.map((t) => ({ ...t, isActive: false })), newTab],
